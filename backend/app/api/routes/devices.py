...
def list_devices(db: Session = Depends(get_database)):
    devices = db.query(Device).order_by(Device.created_at.desc()).all()
    return devices

@router.get("/{devices_id}", response_model=DeviceOut)
def get_device(device_id: int, db: Session = Depends(get_database)):
...
def delete_device(device_id: int, db: Session = Depends(get_database)):
    device = db.query(Device).filter(Device.id == device_id).first()
    if not device:
        raise HTTPException(status_code= 404, detail= "Dispositivo no encontrado")
